...
import { daysUntil, monthlyReportDueDate } from '@/lib/ttb/business-days'

interface Props { distilleryId: string | null }
...
export function OverdueBanner({ distilleryId }: Props) {
  ...
  useEffect(() => {
    ...
    const now = new Date()
    // Prior month's report: due on 15th of current month
    // monthlyReportDueDate(year, month) where month is the month the report IS DUE (current month)
    // but refers to the prior month's activity. Pass current year + current month (1-indexed).
    const reportDue = monthlyReportDueDate(now.getFullYear(), now.getMonth() + 1)
  ...
  if (lateCount > 0) {
    banners.push({ msg: `${lateCount} record${lateCount > 1 ? 's' : ''} entered after the next-business-day deadline (27 CFR 19.580).`, level: 'warn' })
  }
  if (reportDaysLeft !== null && reportDaysLeft <= 7) {
    const level = reportDaysLeft <= 3 ? 'danger' : 'warn'
    const msg = reportDaysLeft <= 0
      ? 'Monthly TTB reports are overdue — file immediately via the Forms tab.'
      : `Monthly TTB reports due in ${reportDaysLeft} day${reportDaysLeft !== 1 ? 's' : ''} — generate them now.`
