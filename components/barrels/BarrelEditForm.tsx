...
export function BarrelEditForm({ barrel, onSave }: { barrel: Barrel; onSave: (b: Barrel) => void }) {
  const [form, setForm] = useState({
    ...
    grain_type: barrel.grain_type || [] as string[],
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <label className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Grain & Mash Bill</label>
          <button 
            onClick={() => {
        ...
        <MaterialPicker 
          ...
          multi
        />
        <Input label="Precise Mash Bill" value={form.mash_bill} onChange={(e) => set('mash_bill', e.target.value)} className="col-span-2" />
      </div>

      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-3">
          <label className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Finish type</label>
          <MaterialPicker 
            category="finish" 
      ...
      <div>
        <label className="block text-sm font-medium text-[var(--color-text)] mb-1 uppercase tracking-wider">Notes</label>
        <textarea
          value={form.notes}
