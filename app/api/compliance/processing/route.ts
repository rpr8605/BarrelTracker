...
export async function GET(req: NextRequest) {
  ...
  let q = admin.from('processing_logs').select('*').eq('distillery_id', distilleryId).order('occurred_at', { ascending: false }).limit(500)
...
export async function POST(req: NextRequest) {
  ...
  const admin = createServiceClient()
  const { data, error } = await admin.from('processing_logs').insert({ ...body, created_by: user.id }).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  return NextResponse.json(data)
}
